...
app.googleSheet =
  "https://docs.google.com/spreadsheets/d/19HR6knMWzdqtdhWucRAa3uXQfLsOO3SuUQE_MPL-LFE/pubhtml";
...
// Pull the game list.
app.getGameList = () => {
  fgcDB.gameData.elements.forEach((item) => {
    gameList.push(item);
  });
...
app.addNotesChangeGame = () => {
  $(`.add-notes-popup select.your-game`).change(function() {
    ...
    $(`.add-notes-popup select.your-character`)
      .empty()
      .append(`<option value="" disabled selected>--Your character--</option>`);
    $(`.add-notes-popup select.opp-character`)
      .empty()
      .append(`<option value="" disabled selected>--Their character--</option>`);
    $(`.add-notes-popup select.note-type`)
      .empty()
      .append(`<option value="" disabled selected>--Note type--</option>`);
